...
var JsonContent = function () {
    ...
    _createClass(JsonContent, [{
        ...
        value: function getArrayTablePaginatorDom(array, type) {
            ...
            // }
            type === 'table' ? getArrayDom = this.getArrayTableDom.bind(this) : getArrayDom = this.getImagesArrayTableDom;
            ...
            var getTR = function getTR(currentValue) {
                var tr = '<tr>\n        ' + Object.keys(currentValue).map(function (key, index, array) {
                        if (index === 2) {
                            // console.log(isModel)
                            if (isModel === true) {
                                return '\n                <td>\n                  <img class="image" src="' + currentValue[key] + '" />\n                </td>';
                            } else {
                                return '\n                <td>\n                  <button class="image view-image-button" src="' + currentValue[key] + '" >view</button>\n                </td>';
                            }
                        }
                        return '<td>' + currentValue[key] + '</td>';
                    }).join('') + '\n      </tr>';
...
var JsonNavigation = function () {
    function JsonNavigation(json) {
        ...
        this.render(json[Object.keys(json)[0]], $('#navigation'));
        this.bindEvent();
    }
...
var JsonShow = function JsonShow(json) {
